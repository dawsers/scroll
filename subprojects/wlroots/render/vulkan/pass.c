	...
	case WLR_RENDER_BLEND_MODE_PREMULTIPLIED:;
		...
		struct wlr_vk_pipeline *pipe = setup_get_or_create_pipeline(
			...
			&(struct wlr_vk_pipeline_key) {
				...
				.layout = { .ycbcr = {0}, .shader_layout = WLR_VK_SHADER_LAYOUT_STANDARD },
			});
		if (!pipe) {
			pass->failed = true;
			break;
...
static void render_pass_add_decoration(struct wlr_render_pass *wlr_pass,
	...
	if (options->clip) {
		...
		pixman_region32_init_rect(&clip, 0, 0, buffer->width, buffer->height);
	}
	...
	case WLR_RENDER_BLEND_MODE_PREMULTIPLIED:;
		...
		struct wlr_vk_pipeline *pipe = setup_get_or_create_pipeline(
			...
			&(struct wlr_vk_pipeline_key) {
				...
				.layout = { .ycbcr = {0}, .shader_layout = WLR_VK_SHADER_LAYOUT_DECORATION },
			});
		if (!pipe) {
			pass->failed = true;
			break;
	...
	case WLR_RENDER_BLEND_MODE_PREMULTIPLIED:;
		...
		struct wlr_vk_pipeline *pipe = setup_get_or_create_pipeline(
			...
			&(struct wlr_vk_pipeline_key) {
				...
				.layout = { .ycbcr = {0}, .shader_layout = WLR_VK_SHADER_LAYOUT_SHADOW },
			});
		if (!pipe) {
			pass->failed = true;
			break;
