...
static void handle_seat_destroy(struct wl_listener *listener, void *data) {
	...
	if (config != NULL && seat == config->handler_context.seat) {
...
static void set_workspace(struct sway_seat *seat,
	...
	// Lua callbacks
	lua_execute_workspace_focus_cbs(new_ws);
}
...
static void seat_set_workspace_focus(struct sway_seat *seat, struct sway_node *node) {
	...
	if (container && container->pending.workspace) {
		if (container->pending.fullscreen_layout == FULLSCREEN_ENABLED &&
			...
			output_layer_shell_enable(container->pending.workspace->output, LAYER_SHELL_OVERLAY);
		} else {
	...
	if (container && container->view) {
		lua_execute_view_focus_cbs(container->view);
