...
def test_focused_inactive_uaf(scroll_compositor: ScrollInstance) -> None:
    # 1. Set mode to vertical to stack windows
    scroll_compositor.cmd("set_mode v")

    # 2. Create first view
    with wayland_client(scroll_compositor, "client1") as client1:
        wait_for_client_map(scroll_compositor, "client1")
        w1_id = scroll_compositor.execute_lua("return scroll.focused_container()")
        print(f"w1_id: {w1_id}")

        # 3. Create second view
        with wayland_client(scroll_compositor, "client2"):
            wait_for_client_map(scroll_compositor, "client2")
            w2_id = scroll_compositor.execute_lua("return scroll.focused_container()")
            print(f"w2_id: {w2_id}")

            # Verify they are siblings (same parent)
            w1_parent = scroll_compositor.execute_lua(
                f"return scroll.container_get_parent({w1_id})"
            )
            w2_parent = scroll_compositor.execute_lua(
            ...
            col1_id = scroll_compositor.execute_lua("""
            ...
            scroll_compositor.cmd(f"[con_id={w1_id}] focus")
            ...
            scroll_compositor.cmd(f"[con_id={w1_id}] kill")
